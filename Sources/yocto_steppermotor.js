...
var Y_USTEPMAXSPEED_INVALID         = YAPI_INVALID_DOUBLE;
...
(function()
{
    function _YStepperMotor(str_func)
    {
        ...
        this._ustepMaxSpeed                  = Y_USTEPMAXSPEED_INVALID;    // MeasureVal
    ...
    function YStepperMotor_parseAttr(name, val, _super)
    {
        ...
        case "stepping":
            ...
            return 1;
        case "ustepMaxSpeed":
            this._ustepMaxSpeed = Math.round(val * 1000.0 / 65536.0) / 1000.0;
            return 1;
        case "overcurrent":
     ...
     * On failure, throws an exception or returns a negative error code.
     */
    function YStepperMotor_set_ustepMaxSpeed(newval)
    {   var rest_val;
        rest_val = String(Math.round(newval * 65536.0));
     ...
     */
    function YStepperMotor_get_ustepMaxSpeed()
    {
        if (this._cacheExpiration <= YAPI.GetTickCount()) {
            if (this.load(YAPI.defaultCacheValidity) != YAPI_SUCCESS) {
    ...
    /**
     * Gets the maximal motor speed for micro-stepping, measured in steps per second.
     *
     * @param callback : callback function that is invoked when the result is known.
     *         The callback function receives three arguments:
     *         - the user-specific context object
     *         - the YStepperMotor object that invoked the callback
     *         - the result:a floating point number corresponding to the maximal motor speed for micro-stepping,
     *         measured in steps per second
     * @param context : user-specific object that is passed as-is to the callback function
     *
     * @return nothing: this is the asynchronous version, that uses a callback instead of a return value
     ...
     * On failure, throws an exception or returns Y_USTEPMAXSPEED_INVALID.
     */
    function YStepperMotor_get_ustepMaxSpeed_async(callback,context)
    {
        var loadcb;                 // func;
        loadcb = function(ctx,obj,res) {
            if (res != YAPI_SUCCESS) {
                callback(context, obj, Y_USTEPMAXSPEED_INVALID);
            } else {
                callback(context, obj, obj._ustepMaxSpeed);
            }
        };
        if (this._cacheExpiration <= YAPI.GetTickCount()) {
            this.load_async(YAPI.defaultCacheValidity,loadcb,null);
    ...
    YStepperMotor = YFunction._Subclass(_YStepperMotor, {
        ...
        STEPPING_FULLSTEP           : 4,
        STEPPING_INVALID            : -1,
        USTEPMAXSPEED_INVALID       : YAPI_INVALID_DOUBLE,
        ...
        set_ustepMaxSpeed           : YStepperMotor_set_ustepMaxSpeed,
        setUstepMaxSpeed            : YStepperMotor_set_ustepMaxSpeed,
        get_ustepMaxSpeed           : YStepperMotor_get_ustepMaxSpeed,
        ustepMaxSpeed               : YStepperMotor_get_ustepMaxSpeed,
        get_ustepMaxSpeed_async     : YStepperMotor_get_ustepMaxSpeed_async,
        ustepMaxSpeed_async         : YStepperMotor_get_ustepMaxSpeed_async,

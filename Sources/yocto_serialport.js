/*********************************************************************
 *
 * $Id: yocto_serialport.js 17610 2014-09-13 11:30:24Z mvuilleu $
 *
...
var Y_MSGCOUNT_INVALID              = YAPI_INVALID_UINT;
...
(function()
{
    function _YSerialPort(str_func)
    {
        ...
        this._msgCount                       = Y_MSGCOUNT_INVALID;         // UInt31
     ...
     */
    function YSerialPort_get_msgCount()
    {
        if (this._cacheExpiration <= YAPI.GetTickCount()) {
            if (this.load(YAPI.defaultCacheValidity) != YAPI_SUCCESS) {
     ...
     * On failure, throws an exception or returns Y_MSGCOUNT_INVALID.
     */
    function YSerialPort_get_msgCount_async(callback,context)
    {
        var loadcb;                 // func;
        loadcb = function(ctx,obj,res) {
            if (res != YAPI_SUCCESS) {
                callback(context, obj, Y_MSGCOUNT_INVALID);
            } else {
                callback(context, obj, obj._msgCount);
            }
        };
        if (this._cacheExpiration <= YAPI.GetTickCount()) {
            this.load_async(YAPI.defaultCacheValidity,loadcb,null);
     ...
     * @return nothing.
     */
    function YSerialPort_read_seek(rxCountVal)
    {
    ...
    YSerialPort = YFunction._Subclass(_YSerialPort, {
        ...
        MSGCOUNT_INVALID            : YAPI_INVALID_UINT,
        ...
        get_msgCount                : YSerialPort_get_msgCount,
        msgCount                    : YSerialPort_get_msgCount,
        get_msgCount_async          : YSerialPort_get_msgCount_async,
        msgCount_async              : YSerialPort_get_msgCount_async,

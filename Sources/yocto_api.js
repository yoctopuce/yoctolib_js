/*********************************************************************
 *
 * $Id: yocto_api.js 13065 2013-10-10 16:04:55Z mvuilleu $
 *
...
(function()
{
    ...
    function YAPI_monitorEvents(int_hubidx)
    {
        ...
        if(yhub.rooturl.substr(0,16) == 'http://callback:') {
    ...
    function YAPI_devRequest(str_device, str_request, func_statechanged, obj_body)
    {
        ...
        if(baseUrl.substr(0,16) == "http://callback:") {
            return this._httpCallbackRequest(method, "/"+devUrl, func_statechanged, obj_body)
        }
    ...
    //
    function YAPI_httpCallbackRequest(method, devUrl, func_statechanged, str_body)
    {
        var async = (typeof func_statechanged != "undefined");
        ...
        if(method == 'POST') {
            var boundary = '???';
            var endb = str_body.indexOf("\r");
            if(str_body.substr(0, 2)=='--' && endb > 2 && endb < 20) {
                boundary = boundary.substr(2, endb-2);
            }
            this._serverResponse.write("\n@YoctoAPI:"+method+" "+devUrl+" "+str_body.length+":"+boundary+"\n"+str_body);
        } else if(method == 'GET') {
            if(devUrl.indexOf('?') == -1 ||
               ...
               devUrl.indexOf('/files.json?a=dir') != -1) {
                ...
                var getmodule = (devUrl.indexOf('api/module.json') != -1);
                if(getmodule) {
                    devUrl = devUrl.replace('api/module.json','api.json');
                }
                if(!this._callbackCache[devUrl]) {
                    console.log(devUrl+" NOT found\n");
                    this._serverResponse.write("\n!YoctoAPI:"+devUrl+" is not preloaded, adding to list");
                    this._serverResponse.write("\n@YoctoAPI:+"+devUrl+"\n");
                    res = { errorType:YAPI_NO_MORE_DATA, 
                            errorMsg:"URL not preloaded, adding to list", 
                            result:null};
                } else {
                    console.log(devUrl+" found\n");
                    var jsonres = this._callbackCache[devUrl];
                    if(getmodule) jsonres = jsonres['module'];
                    if(devUrl.indexOf('.json') != -1) {
        ...
        if(async) {
            var pseudoRequest = { readyState: 4, status: 200, responseText: res.result };
     ...
     * On failure, throws an exception or returns a negative error code.
     */
    function YAPI_RegisterHub_async(str_rooturl, func_callback, obj_context, obj_incomingMessage, obj_serverResponse)
    {
        var i;
        ...
        if(str_rooturl.substr(0,16) == "http://callback:") {
            YAPI._serverResponse = obj_serverResponse;
            var badCallback = function() {
                var errmsg = "RegisterHub(callback) used without posting YoctoAPI data";
                //console.log(errmsg);
                YAPI._serverResponse.write("\n!YoctoAPI:"+errmsg+"\n");
                YAPI._callbackCache = [];
                func_callback(obj_context, YAPI.NO_MORE_DATA, errmsg);
            };
            if(obj_incomingMessage.method != 'POST') {
                badCallback();
            } else {
                YAPI._callbackCache = '';
                obj_incomingMessage.on('end',function() {
                    if(YAPI._callbackCache == "") {
                        badCallback();
                    } else {
                        YAPI._callbackCache = JSON.parse(YAPI._callbackCache);
                        doit();
                    }
                });            
                obj_incomingMessage.on('data',function(chunk) {
                    YAPI._callbackCache += chunk.toString();
                });
            }
        } else {
            doit();
    ...
    function _YAPI()
    {
        ...
        this._httpCallbackRequest       = YAPI_httpCallbackRequest;
...
//--- (end of generated code: return codes)
     ...
     *         - the result:a string corresponding to the commercial name of the module, as set by the facto
     * @param context : user-specific object that is passed as-is to the callback function
     * 
     ...
     *         - the user-specific context object
     *         - the YModule object that invoked the callback
     *         - the result:a string corresponding to the serial number of the module, as set by the facto
     * @param context : user-specific object that is passed as-is to the callback function
     * 
     * @return nothing: this is the asynchronous version, that uses a callback instead of a return value
     * 
     ...
     *         - the user-specific context object
     *         - the YModule object that invoked the callback
     *         - the result:a string corresponding to the logical name of the modu
     * @param context : user-specific object that is passed as-is to the callback function
     * 
     * @return nothing: this is the asynchronous version, that uses a callback instead of a return value
     * 
     ...
     *         - the user-specific context object
     *         - the YModule object that invoked the callback
     *         - the result:an integer corresponding to the USB device identifier of the modu
     * @param context : user-specific object that is passed as-is to the callback function
     * 
     * @return nothing: this is the asynchronous version, that uses a callback instead of a return value
     * 
     ...
     *         - the user-specific context object
     *         - the YModule object that invoked the callback
     *         - the result:an integer corresponding to the hardware release version of the modu
     * @param context : user-specific object that is passed as-is to the callback function
     * 
     * @return nothing: this is the asynchronous version, that uses a callback instead of a return value
     * 
     ...
     *         - the user-specific context object
     *         - the YModule object that invoked the callback
     *         - the result:a string corresponding to the version of the firmware embedded in the modu
     * @param context : user-specific object that is passed as-is to the callback function
     * 
     * @return nothing: this is the asynchronous version, that uses a callback instead of a return value
     * 
     ...
     *         Y_PERSISTENTSETTINGS_MODIFIED corresponding to the current state of persistent module settin
     * @param context : user-specific object that is passed as-is to the callback function
     * 
     * @return nothing: this is the asynchronous version, that uses a callback instead of a return value
     * 
     ...
     *         - the user-specific context object
     *         - the YModule object that invoked the callback
     *         - the result:either Y_BEACON_OFF or Y_BEACON_ON, according to the state of the localization beac
     * @param context : user-specific object that is passed as-is to the callback function
     * 
     * @return nothing: this is the asynchronous version, that uses a callback instead of a return value
     * 
     ...
     *         - the user-specific context object
     *         - the YModule object that invoked the callback
     *         - the result:an integer corresponding to the number of milliseconds spent since the module was powered
     * @param context : user-specific object that is passed as-is to the callback function
     * 
     * @return nothing: this is the asynchronous version, that uses a callback instead of a return value
     * 
     ...
     *         - the user-specific context object
     *         - the YModule object that invoked the callback
     *         - the result:an integer corresponding to the current consumed by the module on the USB bus, in milli-am
     * @param context : user-specific object that is passed as-is to the callback function
     * 
     * @return nothing: this is the asynchronous version, that uses a callback instead of a return value
     * 

/*********************************************************************
 *
 * $Id: yocto_api.js 14799 2014-01-31 14:59:44Z seb $
 *
...
(function()
{
    ...
    function YAPI_monitorEvents(int_hubidx)
    {
        ...
        this.devRequest(yhub.rooturl, "GET /not.byn"+args, 
    ...
    function YAPI_preregisterHub_internal(str_url)
    {
        var rooturl = _cleanRegisteredUrl(str_url);
        
        for(var i = 0; i < this._hubs.length; i++) {
            if(this._hubs[i].rooturl == rooturl) return;
        }
        ...
        var newhub = {
            ...
            rooturl         : rooturl,             // root URL of the hub
    ...
    function YAPI_RegisterHub_async(url, callback, context, incomingMessage, serverResponse)
    {
        ...
        var rooturl = _cleanRegisteredUrl(url);
        var doit = function() {
            YAPI.devRequest_async(rooturl, "GET /api.json", '', function(params,yreq) {
                if(yreq.errorType != YAPI_SUCCESS) {
                    params.cb(params.ctx, yreq.errorType, yreq.errorMsg);
                }
                YAPI._preregisterHub_internal(rooturl);
                // If hub is not yet known, create a device object
                var serial = YAPI._snByUrl[rooturl];
                if(!serial) {
                    // create device asynchronously, without callback
                    new YDevice(rooturl, null, null, function(params,res) {
                        // Update device list
                        YAPI.UpdateDeviceList_async(params.cb, params.ctx);
                    }, params);
                }
            }, {rooturl: rooturl, cb: callback, ctx: context});
        }
        if(rooturl.substr(0,16) == "http://callback:") {
    ...
    function YAPI_UnregisterHub(url)
    {
        ...
        for(i = 0; i < this._hubs.length; i++) {
            if(this._hubs[i].rooturl == rooturl)  {
...
//--- (end of generated code: YFunction class start)
    ...
    function YFunction_get_module()
    {
        ...
        if(this._serial != "") {
            return yFindModule(this._serial);            
        }
        var hwid = this._func;
        var resolve;
        if(hwid.indexOf(".") < 0) {
    ...
    function YFunction_get_module_async(callback, context)
    {
        // try to resolve the function name to a device id without query
        if(this._serial != "") {
            callback(context, this,  yFindModule(this._serial));
            return;
        }
        var hwid = this._func;
        if(hwid.indexOf(".") < 0) {
        ...
        if(dotidx >= 0) {
            // resolution worked
            callback(context, this,  yFindModule(hwid.substr(0, dotidx)));
            return;
        }
...
//--- (end of generated code: YModule class start)
    ...
    YModule = YFunction._Subclass(_YModule, {
        ...
        set_usbBandwidth            : YModule_set_usbBandwidth,
        setUsbBandwidth             : YModule_set_usbBandwidth,
